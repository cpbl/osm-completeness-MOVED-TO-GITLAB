...
parameters {
    ...
    corr_matrix[(L+1)] Omega_psn;               // decomposed correlation matrix for individual-level predictors. 
    corr_matrix[(L+1)] Omega_bin;                  
    vector<lower=0>[L+1] tau_psn;           // scale of diagonals
    vector<lower=0>[L+1] tau_bin;             
    matrix[M+1,L+1] gamma_psn;                // country-level coefficients (including the intercept)
    matrix[M+1,L+1] gamma_bin;
    vector[L+1] beta_psn[J];                  // grid-cell level coefficients (including the intercept). 
    vector[L+1] beta_bin[J];                  // grid-cell level coefficients (including the intercept). 
}
...
transformed parameters {
	...
	row_vector[L+1] z_gamma_psn[J];	
	row_vector[L+1] z_gamma_bin[J];	
	for (j in 1:J) {
	...
	for (i in 1:N){
        lambda[i] <- beta_psn[country[i]][1] + logdensity[i] * beta_psn[country[i]][2];
        theta[i]  <- beta_bin[country[i]][1] + logdensity[i] * beta_bin[country[i]][2];
    }
}
