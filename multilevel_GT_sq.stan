...
data {
    ...
    int<lower=0> L;                     // number of grid-cell level covariates (excl. intercept).
...
parameters {
    ...
    corr_matrix[(L+2)] Omega_psn;               // decomposed correlation matrix for individual-level predictors. +2 is for the intercept and squared term
    corr_matrix[(L+2)] Omega_bin;                  
    vector<lower=0>[L+2] tau_psn;           // scale of diagonals
    vector<lower=0>[L+2] tau_bin;             
    matrix[M+1,L+2] gamma_psn;                // country-level coefficients (including the intercept)
    matrix[M+1,L+2] gamma_bin;
    vector[L+2] beta_psn[J];                  // grid-cell level coefficients (including the intercept). 
    vector[L+2] beta_bin[J];                  // grid-cell level coefficients (including the intercept). 
}
...
transformed parameters {
	...
	row_vector[L+2] z_gamma_psn[J];	
	row_vector[L+2] z_gamma_bin[J];	
	for (j in 1:J) {
	...
	for (i in 1:N){
        lambda[i] <- beta_psn[country[i]][1] + logdensity[i] * beta_psn[country[i]][2] + (logdensity[i]^2) * beta_psn[country[i]][3];
        theta[i]  <- beta_bin[country[i]][1] + logdensity[i] * beta_bin[country[i]][2] + (logdensity[i]^2) * beta_bin[country[i]][3];
    }
}

model {
  	// Weakly informative priors for covariance matrix and second-level parameters
  	// Note the grid-cell results are basically identical with a weaker prior (Cauchy 0,5)
    tau_psn ~ cauchy(0,2);
    tau_bin ~ cauchy(0,2);
    Omega_psn ~ lkj_corr(2.0);
    Omega_bin ~ lkj_corr(2.0);
  	to_vector(gamma_psn) ~ cauchy(0,2);
  	to_vector(gamma_bin) ~ cauchy(0,2);
