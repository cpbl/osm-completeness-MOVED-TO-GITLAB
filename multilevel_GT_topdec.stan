...
data {
    ...
    int<lower=0,upper=1> topdecile[N];   // categorical variable if density is in top decile
...
transformed parameters {
	...
	for (i in 1:N){
        lambda[i] <- beta_psn[country[i]][1] + topdecile[i] * beta_psn[country[i]][2] + logdensity[i] * beta_psn[country[i]][3];
        theta[i]  <- beta_bin[country[i]][1] + topdecile[i] * beta_bin[country[i]][2] + logdensity[i] * beta_bin[country[i]][3];
    }
}
